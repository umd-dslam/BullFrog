...
InitializeLWLocks(void)
{
	...
	/* Initialize hash LWLocks in main array */
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS +
		NUM_BUFFER_PARTITIONS + NUM_LOCK_PARTITIONS + NUM_PREDICATELOCK_PARTITIONS;
	for (id = 0; id < NUM_MIGRATE_JOIN_PARTITIONS; id++, lock++)
			LWLockInitialize(&lock->lock, LWTRANCHE_MIGRATE_JOIN);
...
RegisterLWLockTranches(void)
{
	...
	LWLockRegisterTranche(LWTRANCHE_MIGRATE_JOIN, "migrate_hashtable");
